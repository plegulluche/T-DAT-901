  ...
  return (
    <div className='bg-[#3A3A3A] w-[840px] p-5 rounded-lg mb-5' >
      ...
      <div className='py-10 flex gap-5'>
        <p className='text-[#771Fed] text-4xl cursor-pointer hover:text-amber-500 ml-5'>{content.name}</p>
        <div>
...
const Articles: React.FC<ArticlesProps> = ({ data }) => {
  ...
  return (
    <div className='my-10 gap-10 flex flex-col sm:flex-row bg-[#252525]'>
      <div>
        {data.slice(0, displayCount / 2).map((content, index) => <MainArticle key={index} content={content} />)}
      </div>
